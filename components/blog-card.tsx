...
export function BlogCard({ post }: { post: Post }) {
  return (
    <motion.div whileHover={{ y: -6 }} transition={{ duration: 0.2 }}>
      <Card className="group h-full overflow-hidden border-white/10 bg-gradient-to-br from-white/[0.08] to-white/[0.02]">
        ...
        <div className="relative flex h-full flex-col gap-5">
          ...
          <div className="space-y-3">
            <p className="text-sm uppercase tracking-[0.22em] text-slate-500">{formatDate(post.date)}</p>
            <h3 className="font-heading text-2xl font-semibold tracking-tight text-white">{post.title}</h3>
